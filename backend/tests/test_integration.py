...
class TestAuthApi:
    ...
    async def test_bad_password_is_rejected(self, client: AsyncClient):
        response = await client.post(
            "/api/v1/auth/login", json={"email": "admin@finops.local", "password": "nope"}
        )
    ...
    async def test_viewer_cannot_execute(self, client: AsyncClient, auth: dict):
        created = await client.post(
            ...
            headers=auth,
            json={
                "email": "viewer.test@finops.local",
                "full_name": "Viewer",
        ...
        login = await client.post(
            "/api/v1/auth/login",
            json={"email": "viewer.test@finops.local", "password": "ViewerPassword!2026"},
        )
    ...
    async def test_api_key_authentication(self, client: AsyncClient, auth: dict):
        created = await client.post(
            "/api/v1/auth/api-keys", headers=auth, json={"name": "integration", "rate_limit_per_minute": 500}
        )
        assert created.status_code == 201
...
class TestCatalogueApi:
    ...
    async def test_an_agent_that_ships_is_promoted_out_of_the_roadmap(self):
        ...
        async with SessionFactory() as session:
            agent = (await session.execute(select(Agent).where(Agent.key == "credit_risk"))).scalar_one()
            agent.availability, agent.lifecycle_state = "coming_soon", "disabled"
            agent.config = {"planned_quarter": "Q4 2026"}
            await session.execute(AgentVersion.__table__.delete().where(AgentVersion.agent_id == agent.id))
    ...
    async def test_promotion_does_not_re_enable_an_agent_an_operator_disabled(self):
        ...
        async with SessionFactory() as session:
            agent = (await session.execute(select(Agent).where(Agent.key == "collections"))).scalar_one()
    ...
    async def test_roadmap_agent_cannot_execute(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/agents/trading/execute", headers=auth, json={"input": {"query": "trade"}}
        )
    ...
    async def test_dashboards_respond(self, client: AsyncClient, auth: dict):
        for path in [
            ...
            "/api/v1/dashboard/ai-usage",
            "/api/v1/dashboard/knowledge",
            "/api/v1/dashboard/geography",
...
class TestGeographyDashboard:
    ...
    async def test_countries_come_from_the_transaction_ledger(self, client: AsyncClient, auth: dict, session):
        from sqlalchemy import func, select

        from app.db.models.banking import Transaction

        response = await client.get("/api/v1/dashboard/geography", headers=auth, params={"days": 730})
    ...
    async def test_totals_reconcile_and_shares_are_consistent(self, client: AsyncClient, auth: dict):
        response = await client.get("/api/v1/dashboard/geography", headers=auth, params={"days": 730})
        body = response.json()
    ...
    async def test_high_risk_jurisdictions_follow_the_aml_rule_set(self, client: AsyncClient, auth: dict):
        ...
        high_risk_value = sum(c["total_value"] for c in body["countries"] if c["code"] in HIGH_RISK_COUNTRIES)
        assert body["summary"]["high_risk_value"] == pytest.approx(high_risk_value, rel=1e-6)

    async def test_every_plotted_place_has_real_coordinates(self, client: AsyncClient, auth: dict):
    ...
    async def test_window_narrows_the_result(self, client: AsyncClient, auth: dict):
        wide = (await client.get("/api/v1/dashboard/geography", headers=auth, params={"days": 730})).json()
        narrow = (await client.get("/api/v1/dashboard/geography", headers=auth, params={"days": 1})).json()
    ...
    async def test_lifecycle_transitions(self, client: AsyncClient, auth: dict):
        paused = await client.post(
            "/api/v1/agents/customer_service/lifecycle",
            headers=auth,
            json={"action": "pause", "reason": "test"},
        )
        assert paused.json()["lifecycle_state"] == "paused"
        blocked = await client.post(
            "/api/v1/agents/customer_service/execute", headers=auth, json={"input": {"query": "hi"}}
        )
        assert blocked.status_code == 422
        resumed = await client.post(
            "/api/v1/agents/customer_service/lifecycle", headers=auth, json={"action": "resume"}
        )
    ...
    async def test_config_versioning_and_rollback(self, client: AsyncClient, auth: dict):
        first = await client.put(
            "/api/v1/agents/knowledge_assistant/config",
            headers=auth,
...
class TestBankingTools:
    async def test_authentication_gate_blocks_data_access(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/tools/get_account_balance/invoke", headers=auth, json={"arguments": {}}
        )
        body = response.json()
    ...
    async def test_monitoring_detects_typologies(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/tools/monitor_transactions/invoke",
            headers=auth,
    ...
    async def test_portfolio_analytics(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/tools/analyse_portfolio/invoke",
            headers=auth,
    ...
    async def test_portfolio_risk(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/tools/analyse_portfolio_risk/invoke",
            headers=auth,
            json={"arguments": {"portfolio_code": "PF-BALANCED-01"}},
        )
        data = response.json()["data"]
    ...
    async def test_approval_gated_tool_cannot_be_called_directly(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/tools/escalate_to_human/invoke", headers=auth, json={"arguments": {"reason": "test"}}
        )
        assert response.status_code == 422

    async def test_invalid_arguments_are_rejected(self, client: AsyncClient, auth: dict):
        response = await client.post(
            "/api/v1/tools/get_market_data/invoke", headers=auth, json={"arguments": {"days": 5000}}
        )
...
class TestKnowledge:
    ...
    async def test_document_upload_and_indexing(self, client: AsyncClient, auth: dict):
        ...
        found = await client.post(
            "/api/v1/knowledge/search",
            headers=auth,
            json={"query": "intraday liquidity buffer breach escalation", "top_k": 5},
        )
...
class TestExecutionEngine:
    async def test_full_graph_execution(self, client: AsyncClient, auth: dict, _register_scripted_model):
        provider = _register_scripted_model
        provider.queue_text(
            '{"objective":"Answer the policy question","steps":[],'
            '"required_tools":[],"needs_knowledge_search":true,'
            '"risk_level":"low"}'
        )
        provider.queue_text(
            "A Sev-1 may be declared by the on-call incident commander or any "
        ...
        execution = await _wait_for(client, auth, execution_id, {"succeeded", "failed", "cancelled"})
        ...
        trace = (await client.get(f"/api/v1/executions/{execution_id}/trace", headers=auth)).json()
        assert trace["span_count"] >= 5
        assert trace["root_span_id"]
        kinds = {s["kind"] for s in trace["spans"]}
        assert {"agent", "planner", "retriever", "llm", "validation", "guardrail", "response"} <= kinds
        assert all(s["parent_span_id"] or s["kind"] == "agent" for s in trace["spans"])

        graph = (await client.get(f"/api/v1/executions/{execution_id}/graph", headers=auth)).json()
        assert {n["id"] for n in graph["nodes"]} >= {"planner", "llm", "response"}
        assert any(n["state"] == "executed" for n in graph["nodes"])

        events = (await client.get(f"/api/v1/executions/{execution_id}/events", headers=auth)).json()
    ...
    async def test_tool_calling_execution(self, client: AsyncClient, auth: dict, _register_scripted_model):
        provider = _register_scripted_model
        async with SessionFactory() as session:
        ...
        provider.queue_text(
            '{"objective":"Check the balance","steps":[],'
        ...
        assert {s["name"] for s in tool_spans} == {"tool.authenticate_customer", "tool.get_account_balance"}
        assert all(s["status"] == "ok" for s in tool_spans)

    async def test_human_approval_suspends_and_resumes(
        self, client: AsyncClient, auth: dict, approver_token: str, _register_scripted_model
    ):
        provider = _register_scripted_model
        provider.queue_text(
            '{"objective":"Escalate the complaint","steps":[],'
            '"required_tools":["escalate_to_human"],'
            '"needs_knowledge_search":false,"risk_level":"high"}'
        )
        provider.queue_tool_call(
            "escalate_to_human", {"reason": "Customer alleges unauthorised transaction", "urgency": "high"}
        )
        ...
        operator_login = await client.post(
            "/api/v1/auth/login", json={"email": "operator@finops.local", "password": "TestPassword!2026"}
        )
        ...
        async with SessionFactory() as session:
            ...
            spans = (
                (await session.execute(select(Span).where(Span.execution_id == execution_id))).scalars().all()
            )
            assert any(s.kind == "approval" for s in spans)

    async def test_guardrails_mask_pii(self, client: AsyncClient, auth: dict, _register_scripted_model):
        provider = _register_scripted_model
        provider.queue_text(
            '{"objective":"Reply","steps":[],"required_tools":[],'
            '"needs_knowledge_search":false,"risk_level":"low"}'
        )
    ...
    async def test_provider_failure_is_reported_not_faked(self, client: AsyncClient, auth: dict):
        await client.put(
            "/api/v1/agents/investment_research/config",
            headers=auth,
            json={"model": "gpt-5.5", "changelog": "unconfigured provider"},
        )
        started = await client.post(
            "/api/v1/agents/investment_research/execute",
            headers=auth,
...
class TestGovernance:
    ...
    async def test_feature_flag_update(self, client: AsyncClient, auth: dict):
        response = await client.put(
            "/api/v1/security/feature-flags/hybrid_retrieval",
            headers=auth,
...
async def _wait_for(
    client: AsyncClient, auth: dict, execution_id: str, statuses: set[str], timeout: float = 30.0
) -> dict:
