...
TREATMENT_LADDER: dict[str, dict[str, Any]] = {
    ...
    "2":  {"label": "61-90 days", "actions": ["call", "letter", "hardship_review"],
           "intensity": "elevated"},
    "3":  {"label": "91-180 days", "actions": ["call", "letter", "field_visit",
                                               "restructure_offer"], "intensity": "high"},
    "4":  {"label": "181-365 days", "actions": ["letter", "field_visit", "settlement_offer",
                                                "legal_notice"], "intensity": "high"},
    "5+": {"label": "365+ days", "actions": ["legal_notice", "recovery_agency", "write_off_review"],
           "intensity": "recovery"},
}
...
def asset_classification(dpd: int) -> dict[str, str]:
    ...
    if dpd <= 0:
        return {"classification": "standard", "sub_grade": None,
                "basis": "no amount overdue"}
    if dpd <= SMA_0_MAX_DPD:
        return {"classification": "standard", "sub_grade": "SMA-0",
                "basis": f"overdue {dpd} days (1-30)"}
    if dpd <= SMA_1_MAX_DPD:
        return {"classification": "standard", "sub_grade": "SMA-1",
                "basis": f"overdue {dpd} days (31-60)"}
    if dpd <= SMA_2_MAX_DPD:
        return {"classification": "standard", "sub_grade": "SMA-2",
                "basis": f"overdue {dpd} days (61-90)"}
    if dpd <= SUBSTANDARD_MAX_DPD:
        return {"classification": "sub_standard", "sub_grade": None,
                "basis": f"non-performing for {dpd} days"}
    if dpd <= DOUBTFUL_MAX_DPD:
        return {"classification": "doubtful", "sub_grade": None,
                "basis": f"sub-standard for more than 12 months ({dpd} days overdue)"}
    return {"classification": "loss", "sub_grade": None,
...
def bank_timezone() -> ZoneInfo:
    ...
    try:
        ...
        log.error("invalid_bank_timezone", configured=settings.bank_timezone,
...
async def scan_delinquent_accounts(args: ScanArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    existing = {
        ...
        for case in (await ctx.session.execute(
            select(DelinquencyCase).where(DelinquencyCase.status == "open")
        )).scalars().all()
    }

    if args.facility_type in (None, "loan"):
        loans = (await ctx.session.execute(
            select(Loan).where(Loan.days_past_due >= args.min_days_past_due)
            .order_by(Loan.days_past_due.desc()).limit(args.limit)
        )).scalars().all()
        for loan in loans:
            overdue = round(loan.emi_amount * max(1, loan.days_past_due // 30), 2)
            found.append(_delinquency_row("loan", loan.id, loan.loan_number, loan.customer_id,
                                          loan.outstanding, overdue, loan.emi_amount,
                                          loan.days_past_due, bool(loan.collateral)))

    if args.facility_type in (None, "card"):
        cards = (await ctx.session.execute(
            select(Card).where(Card.status == "active", Card.current_balance > 0)
            .limit(args.limit)
        )).scalars().all()
        for card in cards:
            ...
            if dpd < args.min_days_past_due:
                continue
            found.append(_delinquency_row("card", card.id, card.card_number_masked,
                                          card.customer_id, card.current_balance,
                                          card.minimum_due, card.minimum_due, dpd, False))
    ...
    for row in found:
        entry = by_bucket.setdefault(row["bucket"], {"accounts": 0, "outstanding": 0.0,
                                                     "overdue": 0.0})
        entry["accounts"] += 1
...
def _delinquency_row(facility_type: str, facility_id: str, reference: str, customer_id: str,
                     outstanding: float, overdue: float, minimum_due: float,
                     dpd: int, secured: bool) -> dict[str, Any]:
...
@tool("get_delinquency_case",
      "Fetch a collection case with its arrears, classification, contact history and promises.",
      CaseArgs, category="collections")
async def get_delinquency_case(args: CaseArgs, ctx: ToolContext) -> dict[str, Any]:
    case = await _case(ctx, args.case)
    customer = (await ctx.session.execute(
        select(Customer).where(Customer.id == case.customer_id)
    )).scalar_one_or_none()

    attempts = (await ctx.session.execute(
        select(ContactAttempt).where(ContactAttempt.case_id == case.id)
        .order_by(ContactAttempt.attempted_at.desc()).limit(20)
    )).scalars().all()
    promises = (await ctx.session.execute(
        select(PromiseToPay).where(PromiseToPay.case_id == case.id)
    ...
    return {
        ...
        "treatment": {
            ...
            if case.last_contacted_at else None,
            "next_action_at": _aware(case.next_action_at).isoformat()
            if case.next_action_at else None,
        },
        "contact_history": [
            {"channel": a.channel, "outcome": a.outcome,
             "at": _aware(a.attempted_at).isoformat(), "notes": a.notes}
            for a in attempts
        ],
        "promises": [
            {"amount": p.amount, "promised_date": p.promised_date.isoformat(),
             "status": p.status, "settled_amount": p.settled_amount}
            for p in promises
        ],
    }
...
async def calculate_arrears(args: ArrearsArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    if case.facility_type == "loan":
        loan = (await ctx.session.execute(
            select(Loan).where(Loan.id == case.facility_id)
        )).scalar_one_or_none()
        if loan is None:
        ...
        card = (await ctx.session.execute(
            select(Card).where(Card.id == case.facility_id)
        )).scalar_one_or_none()
        if card is None:
            raise NotFoundError("The facility behind this case no longer exists")
...
@tool("assess_hardship",
      ...
      HardshipArgs, category="collections")
async def assess_hardship(args: HardshipArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    if args.verify_from_ledger:
        account_ids = (await ctx.session.execute(
            select(Account.id).where(Account.customer_id == case.customer_id)
        )).scalars().all()
        if account_ids:
    ...
    other_obligations = float((await ctx.session.execute(
        select(func.coalesce(func.sum(Loan.emi_amount), 0.0)).where(
            Loan.customer_id == case.customer_id, Loan.status == "active",
            Loan.id != case.facility_id)
    )).scalar_one())
    ...
    return {
        ...
        "reason": args.reason,
        "recommendation": (
            "propose a plan within the surplus" if affordable
...
async def score_collectability(args: CollectabilityArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    reached = sum(1 for a in attempts if a.outcome in {"contacted", "promise_captured",
                                                       "payment_made"})
    ...
    if account_ids:
        since = datetime.now(UTC) - timedelta(days=90)
        inflow = float((await ctx.session.execute(
            select(func.coalesce(func.sum(func.abs(Transaction.amount)), 0.0)).where(
                Transaction.account_id.in_(account_ids),
                Transaction.booked_at >= since, Transaction.direction == "credit")
        )).scalar_one())
    ...
    return {
        ...
        "evidence": {
            "promises_kept": kept, "promises_broken": broken,
            "contact_attempts": len(attempts), "contacts_reached": reached,
            "average_monthly_inflow": round(monthly_inflow, 2),
            "amount_overdue": case.amount_overdue,
        },
        "omitted_components": [
            name for name in ("promise_performance", "contact_responsiveness", "inflow_coverage")
    ...
    if case.cease_contact:
        for action in ("sms", "email", "call", "field_visit"):
            if action in allowed:
                ...
                suppressed.append({"action": action,
                                   "reason": "customer issued a cease-contact instruction"})
    ...
    if case.dispute_open:
        for action in ("legal_notice", "recovery_agency", "settlement_offer", "field_visit"):
            if action in allowed:
                allowed.remove(action)
                suppressed.append({"action": action,
                                   "reason": "collection activity is paused while a dispute "
                                             "is open"})
    if case.hardship_flag:
        for action in ("legal_notice", "recovery_agency", "field_visit"):
            if action in allowed:
                allowed.remove(action)
                suppressed.append({"action": action,
                                   "reason": "customer is in a hardship arrangement"})

    score = args.collectability_score
    if score is not None and score < 0.33 and "restructure_offer" not in allowed \
...
class EligibilityArgs(BaseModel):
    ...
    proposed_at: str | None = Field(default=None,
                                    description="ISO timestamp of the proposed contact; "
                                                "defaults to now")


@tool("check_contact_eligibility",
      ...
      EligibilityArgs, category="collections")
async def check_contact_eligibility(args: EligibilityArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    if case.cease_contact:
        blockers.append({"rule": "cease_contact",
    ...
    if case.dispute_open and args.channel != "letter":
        blockers.append({"rule": "dispute_open",
                         "detail": "only written correspondence is permitted while a dispute "
                                   "is open"})
    ...
    recent = (await ctx.session.execute(
        select(ContactAttempt).where(
            ContactAttempt.case_id == case.id, ContactAttempt.attempted_at >= week_start)
    ...
    if len(live) >= MAX_ATTEMPTS_PER_WEEK:
        blockers.append({"rule": "weekly_frequency_cap",
                         "detail": f"{len(live)} attempts in the last 7 days, cap "
                                   f"{MAX_ATTEMPTS_PER_WEEK}"})
    same_day = [a for a in live if _aware(a.attempted_at).date() == when.date()]
    if len(same_day) >= MAX_ATTEMPTS_PER_DAY:
        blockers.append({"rule": "daily_frequency_cap",
                         "detail": f"{len(same_day)} attempt(s) already today, cap "
    ...
    if live:
        ...
        if hours < MIN_HOURS_BETWEEN_ATTEMPTS:
            blockers.append({"rule": "minimum_interval",
                             "detail": f"{hours:.1f}h since the last attempt, minimum "
                                       f"{MIN_HOURS_BETWEEN_ATTEMPTS}h"})
    ...
    if blockers and not case.cease_contact and case.contact_consent:
        ...
        if live:
            candidate = max(candidate,
                            to_local(_aware(live[0].attempted_at))
                            + timedelta(hours=MIN_HOURS_BETWEEN_ATTEMPTS))
        if candidate.time() < CONTACT_WINDOW_START:
            candidate = candidate.replace(hour=CONTACT_WINDOW_START.hour,
                                          minute=CONTACT_WINDOW_START.minute,
                                          second=0, microsecond=0)
        elif candidate.time() > CONTACT_WINDOW_END:
            candidate = (candidate + timedelta(days=1)).replace(
                hour=CONTACT_WINDOW_START.hour, minute=CONTACT_WINDOW_START.minute,
                second=0, microsecond=0)
    ...
    return {
        ...
        "next_eligible_at": None if case.cease_contact or not case.contact_consent
        else next_allowed,
...
class LogContactArgs(BaseModel):
    ...
    outcome: str = Field(description="contacted | no_answer | wrong_number | refused | "
                                     "promise_captured | payment_made | cease_requested")
    notes: str | None = Field(default=None)


@tool("log_contact_attempt",
      "Record an outreach attempt against the case. A cease request stops all future contact.",
      LogContactArgs, category="collections", writes_data=True, idempotent=False)
async def log_contact_attempt(args: LogContactArgs, ctx: ToolContext) -> dict[str, Any]:
    case = await _case(ctx, args.case)
    now = datetime.now(UTC)
    attempt = ContactAttempt(
        case_id=case.id, customer_id=case.customer_id, channel=args.channel,
        outcome=args.outcome, attempted_at=now, local_hour=to_local(now).hour,
        agent_name=ctx.user_email, notes=args.notes, execution_id=ctx.execution_id,
    )
...
@tool("record_promise_to_pay",
      ...
      PromiseArgs, category="collections", writes_data=True, idempotent=False)
async def record_promise_to_pay(args: PromiseArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    if args.amount > case.outstanding:
        raise ValidationError(
            f"Promise of {args.amount:,.2f} exceeds the outstanding balance "
            f"of {case.outstanding:,.2f}")

    promise = PromiseToPay(
        case_id=case.id, customer_id=case.customer_id, amount=args.amount,
        promised_date=promised, channel=args.channel, status="pending",
        captured_by=ctx.user_email, execution_id=ctx.execution_id,
    )
...
class PromisePerformanceArgs(BaseModel):
    case: str = Field(description="Case number or id")
    settle_due: bool = Field(default=True,
                             description="Mark past-dated promises kept or broken from the ledger")


@tool("evaluate_promise_performance",
      "Check promises against the ledger and mark each one kept or broken.",
      PromisePerformanceArgs, category="collections", writes_data=True)
async def evaluate_promise_performance(args: PromisePerformanceArgs,
    ...
    if not promises:
        return {"case_number": case.case_number, "promises": 0,
                "note": "no promises recorded on this case"}
    ...
    for promise in promises:
        paid = 0.0
        if account_ids:
            window_start = datetime.combine(promise.promised_date - timedelta(days=3),
                                            time.min, tzinfo=UTC)
            window_end = datetime.combine(promise.promised_date + timedelta(days=3),
                                          time.max, tzinfo=UTC)
            paid = float((await ctx.session.execute(
                select(func.coalesce(func.sum(func.abs(Transaction.amount)), 0.0)).where(
                    Transaction.account_id.in_(account_ids),
        ...
        if args.settle_due and promise.status == "pending" \
                and promise.promised_date < date.today():
    ...
    return {
        ...
        "detail": results,
        "matching": "repayments within +/- 3 days of the promised date, categorised "
                    "loan_repayment",
    }
...
async def create_repayment_plan(args: PlanArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    if args.instalment_amount < MIN_PLAN_INSTALMENT:
        raise ValidationError(
            f"Instalment is below the minimum viable amount of {MIN_PLAN_INSTALMENT:,.2f}")
    try:
    ...
    plan = RepaymentPlan(
        ...
        case_id=case.id, customer_id=case.customer_id,
        plan_type="settlement" if args.concession_type == "settlement" else "instalment",
        instalment_amount=args.instalment_amount, instalments=args.instalments,
        frequency="monthly", first_payment_date=first, total_payable=total_payable,
        concession_type=args.concession_type, concession_value=args.concession_value,
        affordability={"surplus_available": args.surplus_available,
                       "utilisation_pct": round(
                           args.instalment_amount / args.surplus_available * 100, 2)
                       if args.surplus_available else None},
        status="proposed", approved_by=ctx.user_email, execution_id=ctx.execution_id,
    )
...
@tool("escalate_to_recovery",
      ...
      RecoveryArgs, category="collections", writes_data=True,
      requires_approval=True, approval_risk="critical", idempotent=False)
...
async def collections_portfolio_summary(args: PortfolioArgs, ctx: ToolContext) -> dict[str, Any]:
    ...
    promises = (await ctx.session.execute(
        select(PromiseToPay).where(PromiseToPay.created_at >= since)
    )).scalars().all()

    by_bucket: dict[str, dict[str, Any]] = {}
    provision_total = 0.0
    for case in cases:
        entry = by_bucket.setdefault(case.bucket, {"cases": 0, "outstanding": 0.0,
                                                   "overdue": 0.0, "provision": 0.0})
    ...
    return {
        ...
        "provision_coverage_pct": round(provision_total / outstanding * 100, 2)
        if outstanding else 0.0,
        "npa_cases": len(npa),
        "npa_outstanding": round(sum(case.outstanding for case in npa), 2),
        "by_bucket": {
            bucket: {"cases": data["cases"],
                     "label": TREATMENT_LADDER.get(bucket, {}).get("label"),
                     "outstanding": round(data["outstanding"], 2),
                     "overdue": round(data["overdue"], 2),
