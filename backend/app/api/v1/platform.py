...
async def list_tools(session: SessionDep, principal: PrincipalDep) -> list[dict[str, Any]]:
    ...
    health_rows = {h.tool_name: h for h in (await session.execute(select(ToolHealth))).scalars().all()}
    out = []
        ...
        out.append(
            {
                ...
                "health": {
                    ...
                    if health and health.total_calls
                    else 100.0,
                    "last_called_at": health.last_called_at.isoformat()
                    if health and health.last_called_at
                    else None,
...
async def invoke_tool(
    tool_name: str, payload: ToolInvokeRequest, request: Request, session: SessionDep, principal: PrincipalDep
) -> dict[str, Any]:
    ...
    result = await tool_registry.invoke(
        ...
        payload.arguments,
        ToolContext(
            user_id=principal.id, user_email=principal.email, roles=principal.roles, session=session, state={}
        ),
    )
    await write_audit(
        session,
        ...
        details={"arguments": payload.arguments},
        request=request,
    )
...
async def _probe(name: str, category: str, coro) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "name": name,
            "category": category,
            "status": "healthy" if result else "not_configured",
...
async def _kafka_status() -> dict[str, Any]:
    ...
    return {
        "status": "healthy" if bus.kafka_connected else "unhealthy",
...
async def _neo4j_status() -> dict[str, Any]:
    if not (settings.neo4j_uri and settings.neo4j_user):
        return {"status": "not_configured", "required": ["NEO4J_URI", "NEO4J_USER", "NEO4J_PASSWORD"]}
    try:
...
async def _elastic_status() -> dict[str, Any]:
    ...
    return {"status": "healthy" if resp.status_code < 400 else "unhealthy", "http_status": resp.status_code}
...
async def connected_services(principal: PrincipalDep) -> dict[str, Any]:
    ...
    infra = await asyncio.gather(
        ...
        _probe(
            ...
            asyncio.sleep(
                ...
                result={
                    ...
                    "required": []
                    if store.healthy
                    else ["MINIO_ENDPOINT", "MINIO_ACCESS_KEY", "MINIO_SECRET_KEY"],
                },
            ),
        ),
        _probe(
        ...
        _probe(
            "prometheus",
            "observability",
            _http_probe("prometheus", None, ["scrape /metrics from this service"]),
        ),
        _probe(
            "jaeger",
            "observability",
    ...
    return {
        ...
        "summary": {
            ...
            "unhealthy": sum(1 for s in all_services if s.get("status") in {"unhealthy", "degraded"}),
        },
...
async def list_sources(session: SessionDep, principal: PrincipalDep) -> list[dict[str, Any]]:
    ...
    rows = (await session.execute(select(KnowledgeSource).order_by(KnowledgeSource.name))).scalars().all()
    return [
        {
            "id": s.id,
            "key": s.key,
...
async def create_source(
    payload: SourceCreate, session: SessionDep, principal: PrincipalDep
) -> dict[str, Any]:
...
async def sync_source(key: str, session: SessionDep, principal: PrincipalDep) -> dict[str, Any]:
    ...
    await write_audit(
        session,
        principal=principal,
        action="knowledge.source.synced",
        resource_type="knowledge_source",
        resource_id=key,
        details=result,
    )
...
async def upload_document(
    ...
    try:
        ...
        if file.content_type == "application/pdf":
            ...
            raise ValidationError("Only UTF-8 text or PDF uploads are supported") from None

    stored = await store.put(f"knowledge/{key}/{file.filename}", raw, file.content_type or "text/plain")
    document = await pipeline.ingest_document(
        ...
        metadata={"sha256": stored["sha256"], "size_bytes": stored["size_bytes"]},
    )
...
async def search_knowledge(
    payload: SearchRequest, session: SessionDep, principal: PrincipalDep
) -> dict[str, Any]:
    principal.require(Permission.KNOWLEDGE_READ)
    result = await pipeline.search(
        session, payload.query, top_k=payload.top_k, source_keys=payload.sources or None
    )
    return {
        "query": payload.query,
...
async def run_playground(
    payload: PlaygroundRequest, session: SessionDep, principal: PrincipalDep
) -> dict[str, Any]:
    ...
    return {
        ...
        "comparison": {
            "fastest": min(successful, key=lambda r: r["latency_ms"])["model"] if successful else None,
            "cheapest": min(successful, key=lambda r: r["cost_usd"])["model"] if successful else None,
            "longest_output": max(successful, key=lambda r: r["characters"])["model"] if successful else None,
        },
    }
...
async def list_playground_runs(
    session: SessionDep, principal: PrincipalDep, limit: int = Query(default=25, ge=1, le=200)
) -> list[dict[str, Any]]:
    principal.require(Permission.PLAYGROUND_USE)
    rows = (
        (await session.execute(select(PlaygroundRun).order_by(PlaygroundRun.created_at.desc()).limit(limit)))
        .scalars()
        .all()
    )
    return [
        ...
        "models": [
            {
                ...
                "available": spec.provider in configured,
                "tags": list(spec.tags),
            }
...
async def submit_feedback(
    execution_id: str, payload: FeedbackRequest, session: SessionDep, principal: PrincipalDep
) -> dict[str, Any]:
    ...
    evaluation = (
        await session.execute(
            select(Evaluation).where(Evaluation.execution_id == execution_id, Evaluation.evaluator == "human")
        )
    ).scalar_one_or_none()
    if evaluation is None:
        evaluation = Evaluation(execution_id=execution_id, agent_key=execution.agent_key, evaluator="human")
...
async def run_evaluation(execution_id: str, session: SessionDep, principal: PrincipalDep) -> dict[str, Any]:
