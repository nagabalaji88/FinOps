...
class DailyCost(TypedDict):
    """One day of the cost series. Named so `cost_usd` stays a float where it is summed."""
...
async def executive_overview(session: SessionDep, principal: PrincipalDep) -> dict[str, Any]:
    ...
    return {
        ...
        "cost": {
            ...
            if settings.daily_cost_budget_usd
            else 0.0,
            "monthly_budget_used_pct": round(monthly_cost / settings.monthly_cost_budget_usd * 100, 2)
            if settings.monthly_cost_budget_usd
            else 0.0,
        },
        "approvals": {
...
async def ai_usage(
    session: SessionDep, principal: PrincipalDep, days: int = Query(default=7, ge=1, le=90)
) -> dict[str, Any]:
    principal.require(Permission.METRIC_READ)
    ...
    by_model = (
        await session.execute(
            select(
                ...
                func.sum(CostRecord.tokens_input),
                func.sum(CostRecord.tokens_output),
                func.sum(CostRecord.tokens_cached),
                func.sum(CostRecord.cost_usd),
                func.avg(CostRecord.latency_ms),
            )
    ...
    search_stats = (
        await session.execute(
            select(func.count(SearchQueryLog.id), func.avg(SearchQueryLog.latency_ms)).where(
                SearchQueryLog.timestamp >= since
            )
        )
    ).one()
...
async def execution_timeseries(
    session: SessionDep, principal: PrincipalDep, hours: int = Query(default=24, ge=1, le=720)
) -> dict[str, Any]:
    principal.require(Permission.METRIC_READ)
...
async def cost_summary(
    session: SessionDep, principal: PrincipalDep, days: int = Query(default=30, ge=1, le=365)
) -> dict[str, Any]:
    ...
    daily_rows = (
        await session.execute(
            select(func.date(CostRecord.timestamp), func.sum(CostRecord.cost_usd), func.count(CostRecord.id))
            .where(CostRecord.timestamp >= since)
            .group_by(func.date(CostRecord.timestamp))
        )
    ).all()
    daily_series: list[DailyCost] = sorted(
        [
            DailyCost(date=str(d), cost_usd=round(float(c or 0), 6), calls=int(n or 0))
    ...
    days_remaining = (
        (month_start.replace(month=month_start.month % 12 + 1, day=1) - now).days
        if month_start.month != 12
        else (month_start.replace(year=month_start.year + 1, month=1, day=1) - now).days
    )
    ...
    if settings.daily_cost_budget_usd and daily > settings.daily_cost_budget_usd * 0.8:
        alerts.append(
            {
                "severity": "warning" if daily < settings.daily_cost_budget_usd else "critical",
                "scope": "daily",
                "message": f"Daily spend ${daily:.2f} against budget ${settings.daily_cost_budget_usd:.2f}",
            }
        )
...
async def platform_metrics(session: SessionDep, principal: PrincipalDep) -> dict[str, Any]:
    ...
    executions_last_hour = int(
        (
            await session.execute(select(func.count(Execution.id)).where(Execution.created_at >= hour_ago))
        ).scalar_one()
    )
    tokens_last_minute = int(
        (
            await session.execute(
                select(func.sum(Execution.tokens_input + Execution.tokens_output)).where(
                    Execution.created_at >= minute_ago
                )
            )
        ).scalar_one()
        or 0
    ...
    latencies = (
        (
            await session.execute(
                select(Execution.latency_ms)
                .where(Execution.latency_ms.is_not(None), Execution.created_at >= hour_ago)
    ...
    queue = int(
        (
            await session.execute(select(func.count(Execution.id)).where(Execution.status == "queued"))
        ).scalar_one()
    )
    retries = int(
        (
...
async def knowledge_dashboard(session: SessionDep, principal: PrincipalDep) -> dict[str, Any]:
    ...
    embedded = int(
        (await session.execute(select(func.count(Chunk.id)).where(Chunk.embedding.is_not(None)))).scalar_one()
    )
    dims = (
    ...
    search = (
        await session.execute(
            select(
                ...
                func.avg(SearchQueryLog.latency_ms),
        ...
        "sources": [
            {
                ...
                "vector_dimensions": s.vector_dimensions,
...
async def geography_dashboard(
    session: SessionDep, principal: PrincipalDep, days: int = Query(default=90, ge=1, le=730)
) -> dict[str, Any]:
    ...
    watchlist_rows = (
        await session.execute(
            select(
                SanctionsEntry.nationality,
                func.count(SanctionsEntry.id),
    ...
    security_rows = (
        await session.execute(select(Security.country, func.count(Security.id)).group_by(Security.country))
    ).all()
    securities = {
        code: int(total)
    ...
    customer_country_rows = (
        await session.execute(
            select(Customer.nationality, func.count(Customer.id)).group_by(Customer.nationality)
        )
    ).all()
    ...
    for code, count, value, inbound, outbound, flagged, customer_count in country_rows:
        ...
        country_value = float(value or 0.0)
        country_count = int(count or 0)
        if resolved != geo.DOMESTIC_COUNTRY:
            cross_border_value += country_value
            cross_border_transactions += country_count
        if risk == "high":
            high_risk_value += country_value
            high_risk_transactions += country_count
        countries.append(
            {
                "code": resolved,
                "name": place.name,
                ...
                "risk_level": risk,
                "transactions": country_count,
                "total_value": round(country_value, 2),
                ...
                "share_pct": round(country_value / total_value * 100, 2) if total_value else 0.0,
            }
        )
    ...
    high_risk_customers: dict[str, int] = {
        city: count
        for city, count in (
            await session.execute(
        ...
        cities.append(
            {
                ...
                "total_value": round(city_value, 2),
                "cross_border_value": round(float(cross_value or 0.0), 2),
                "flagged": int(flagged or 0),
                "alerts": int(alerts_by_city.get(name, 0)),
                "share_pct": round(city_value / total_value * 100, 2) if total_value else 0.0,
            }
        )
    ...
    for city_name, code, count, value, outbound, flagged in corridor_rows:
        ...
        if origin is None or destination is None or resolved is None:
            continue
        corridor_value = float(value or 0.0)
        corridor_outbound = float(outbound or 0.0)
        corridors.append(
            {
                ...
                "risk_level": _risk_level(resolved, watchlist=watchlist.get(resolved, {}).get("entries", 0)),
                "transactions": int(count or 0),
                "total_value": round(corridor_value, 2),
                "outbound_value": round(corridor_outbound, 2),
                "inbound_value": round(corridor_value - corridor_outbound, 2),
                "flagged": int(flagged or 0),
            }
        )
    ...
    trend_rows = (
        await session.execute(
            select(Transaction.booked_at, Transaction.country, amount).where(Transaction.booked_at >= since)
        )
    ).all()
    buckets: dict[str, dict[str, Any]] = {}
    for booked_at, code, value in trend_rows:
        ...
        day_bucket = buckets.setdefault(
            key,
            {
                ...
                "transactions": 0,
                "domestic_value": 0.0,
                "cross_border_value": 0.0,
                "high_risk_value": 0.0,
            },
        )
        day_bucket["transactions"] += 1
        resolved = geo.resolve(code) or ""
        txn_value = float(value or 0.0)
        if resolved == geo.DOMESTIC_COUNTRY:
            day_bucket["domestic_value"] += txn_value
        else:
            day_bucket["cross_border_value"] += txn_value
        if _risk_level(resolved, watchlist=0) == "high":
            day_bucket["high_risk_value"] += txn_value
    trend = [
        {
            **bucket,
    ...
    alerts = int(
        (
            await session.execute(select(func.count(AmlAlert.id)).where(AmlAlert.detected_at >= since))
        ).scalar_one()
    )
    flagged_total = int(
        (
            await session.execute(
                select(func.count(Transaction.id)).where(
                    Transaction.booked_at >= since, Transaction.is_flagged.is_(True)
                )
            )
        ).scalar_one()
    )
    currency = (
        await session.execute(select(Transaction.currency).where(Transaction.booked_at >= since).limit(1))
    ).scalar_one_or_none()
    ...
    return {
        ...
        "summary": {
            ...
            "high_risk_share_pct": round(high_risk_value / total_value * 100, 2) if total_value else 0.0,
