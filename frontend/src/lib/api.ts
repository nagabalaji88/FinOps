...
/**
 * Subscribe to an execution's SSE stream. EventSource cannot set headers, so the
 * access token is passed as a query parameter that the API also accepts.
 */
      ...
      for (;;) {
        ...
        for (const frame of frames) {
          ...
          try {
            ...
            if (payload?.type === 'heartbeat' || payload?.timestamp === undefined) {
              if (!payload?.type) continue
            }
            handlers.onEvent(payload as ExecutionEvent)
          } catch {
